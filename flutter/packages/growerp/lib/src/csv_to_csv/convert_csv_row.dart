...
List<String> convertCsvRow(FileType fileType, List<String> columnsFrom,
  ...
  switch (fileType) {
    ...
    case FileType.glAccount:
      ...
      if (columnsFrom[8] == 'Ending Balance') {
        columnsTo.add(columnsFrom[0]); //0 accountCode
        columnsTo.add(columnsFrom[1]); //1 account name
        columnsTo.add('');
        columnsTo.add(columnsFrom[5]);
    ...
    case FileType.finDocTransaction:
      if (fileName.toLowerCase().contains('balance')) {
        ...
        if (startDate != null) {
          columnsTo.add(
              "${startDate.toString().substring(0, 10)}-@@@@@@"); // will be replaced by sequential id
          columnsTo.add("true");
          columnsTo.add('Transaction');
          columnsTo.add('Initial ledger posted values');
          columnsTo.add(startDate.toString().substring(0, 10));
          columnsTo.add('');
          columnsTo.add('');
          columnsTo.add('');
    ...
    case FileType.finDocTransactionItem:
      ...
      if (fileName.toLowerCase().contains('balance')) {
        ...
        if (columnsFrom[3] != '' || columnsFrom[4] != '') {
          if (columnsFrom[0] == '') return ([]);
          columnsTo.add(
              "${startDate.toString().substring(0, 10)}-@@@@@@"); // will be replaced by sequential id
          columnsTo.add('Transaction');
          ...
          columnsTo.add(columnsFrom[0]);
          columnsTo.add(columnsFrom[3] != '' ? 'true' : 'false');
        }
        return columnsTo;
      } else {
